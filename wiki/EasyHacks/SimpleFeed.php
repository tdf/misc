...
function parseFeed($input, $args, $parser)
{
	...
	// don't strip class attribute
	$strip_attribs = $feed->strip_attributes;
	array_splice($strip_attribs, array_search('class', $strip_attribs), 1);
	$feed->strip_attributes($strip_attribs);
	...
	for ($i = 0; $i < $max; $i++)
	{
		...
		// id contains the URL to the bug
		$itemwikitext = str_replace('{ID}', $item->get_id(), $itemwikitext);
		...
		if (array_key_exists('type', $args) && $args['type'] == 'planet')
		{
